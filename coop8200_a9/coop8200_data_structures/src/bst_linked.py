...
class BST:
    ...
    def is_balanced(self):
        ...
        height between a nodes left and right subtrees is <= 1.
        Use: b = bst.is_balanced()
        ---------------------------------------------------------
        Postconditions:
          returns:
          balanced - True if the bst is balanced, False otherwise
        ---------------------------------------------------------
        """
    ...
    def is_balanced_aux(self, node):
        ...
        if balanced != False:
            if node._left != None and node._right != None:
                difference = node._left._height - node._right._height
                if -1 <= difference <= 1:
                    self.is_balanced_aux(node._left)
                    self.is_balanced_aux(node._right)
                    balanced = True
                else:
                    balanced = False
            elif node._left == None and node._right != None:
                difference = 0 - node._right._height
                if -1 <= difference <= 1:
                    self.is_balanced_aux(node._right)
                    balanced = True
                else:
                    balanced = False
            elif node._right == None and node._left != None:
                difference = node._left._height - 0
                if -1 <= difference <= 1:
                    self.is_balanced_aux(node._left)
                    balanced = True
                else:
    ...
    def node_counts_aux(self, node):
        ...
        if node != None:
            if node._left != None and node._right != None:
                ...
                count[1] = 1
            count = [x+y for x,y in zip(count, self.node_counts_aux(node._left))]
            count = [x+y for x,y in zip(count, self.node_counts_aux(node._right))]
